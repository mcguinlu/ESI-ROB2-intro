...
E.g. if reasons for drop-out are recorded, then this can provide evidence that missingness is related to true value
...
Sensitivity analyses make assumptions about the relationship between outcome missingness and the true value

If the results do not change under different __assumptions__ about missing outcome data, then risk of bias will be .g[**Low**]
...
Occurs when measured value $\neq$ true value

Can be __differential__ or __non-differential__

Common terms include measurement error and misclassification
...
Impact on different outcome types:
...
Not always __feasible__, e.g. patient reported outcomes
